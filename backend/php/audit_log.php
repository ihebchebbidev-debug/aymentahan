...
try {
    ...
    $actions  = $db->query("SELECT DISTINCT action FROM crminternet_audit_log ORDER BY action")->fetchAll(PDO::FETCH_COLUMN);
    $entities = $db->query("SELECT DISTINCT entity_type FROM crminternet_audit_log WHERE entity_type IS NOT NULL ORDER BY entity_type")->fetchAll(PDO::FETCH_COLUMN);
