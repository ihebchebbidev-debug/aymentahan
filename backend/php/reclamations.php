...
function ensure_reclamations_tables(PDO $db): void {
    ...
    $repair = [
        "ALTER TABLE crminternet_reclamations ADD COLUMN mois TINYINT UNSIGNED NULL",
        "ALTER TABLE crminternet_reclamations ADD COLUMN annee SMALLINT UNSIGNED NULL",
    ];
...
/** mois/annee : colonnes normales (MariaDB < 10.2 n'a pas les colonnes GENERATED). */
function rec_sync_period(array &$p): void {
    if (empty($p['date_creation'])) {
        return;
    }
    $t = strtotime($p['date_creation']);
    if (!$t) {
        return;
    }
    $p['mois']  = (int)date('n', $t);
    $p['annee'] = (int)date('Y', $t);
}
...
if ($method === 'GET') {
    ...
    ok(['reclamations' => $rows, 'total' => $total, 'limit' => $limit, 'offset' => $offset,
