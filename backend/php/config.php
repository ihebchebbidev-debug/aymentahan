...
function audit_log(
    ...
    try {
        ...
        $s = $db->prepare('INSERT INTO crminternet_audit_log
            (created_at, user_username, user_role, action, entity_type, entity_id, method, path, ip, user_agent, status_code, details)
            VALUES (NOW(), :u, :r, :a, :et, :eid, :m, :p, :ip, :ua, :sc, :d)');
        $s->execute([
...
function admin_copy_emails(PDO $db): array {
    // 1) Settings: otp_admin_copy_emails (preferred) or legacy admin_copy_emails.
    foreach (['otp_admin_copy_emails', 'admin_copy_emails'] as $settingKey) {
        try {
            $s = $db->prepare(
                "SELECT value FROM crminternet_settings WHERE scope='global' AND setting_key = :k LIMIT 1"
            );
            $s->execute([':k' => $settingKey]);
            $val = $s->fetchColumn();
            if ($val) {
        ...
        } catch (Throwable $e) { /* table or row missing — try next key */ }
    }
...
/** Best-effort client IP (Cloudflare / proxy / direct). Used for OTP allowlist + audit. */
...
function ensure_audit_log_table(PDO $db): void {
    ...
    // Legacy installs may miss created_at or contain NULL / zero dates.
    // Avoid CURRENT_TIMESTAMP defaults here because some older MySQL/MariaDB
    // setups reject them during ALTER TABLE and break /audit entirely.
    try {
        $row = $db->query("SELECT DATA_TYPE, IS_NULLABLE
        ...
        if (!$row) {
            ...
            $row = ['DATA_TYPE' => 'datetime', 'IS_NULLABLE' => 'YES'];
        }
        ...
        if (strtolower((string)($row['DATA_TYPE'] ?? '')) !== 'datetime'
            || (string)($row['IS_NULLABLE'] ?? '') === 'YES') {
            $db->exec("ALTER TABLE crminternet_audit_log
                       MODIFY COLUMN created_at DATETIME NOT NULL");
        }
    } catch (Throwable $e) { /* best-effort legacy repair */ }
}
