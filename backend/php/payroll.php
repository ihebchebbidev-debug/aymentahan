...
function ensure_payroll(PDO $db): void {
    try {
        ...
        $repair = [
            "ALTER TABLE crminternet_payroll CHANGE COLUMN period_month period CHAR(7) NOT NULL",
            "ALTER TABLE crminternet_payroll ADD COLUMN username VARCHAR(80) NOT NULL DEFAULT ''",
            "ALTER TABLE crminternet_payroll ADD COLUMN period CHAR(7) NOT NULL DEFAULT '2000-01'",
            "ALTER TABLE crminternet_payroll ADD COLUMN hours_worked DECIMAL(7,2) NOT NULL DEFAULT 0",
            "ALTER TABLE crminternet_payroll ADD COLUMN hourly_rate DECIMAL(8,2) NOT NULL DEFAULT 0",
            "ALTER TABLE crminternet_payroll ADD COLUMN bonus DECIMAL(10,2) NOT NULL DEFAULT 0",
            "ALTER TABLE crminternet_payroll ADD COLUMN total DECIMAL(10,2) NOT NULL DEFAULT 0",
            "ALTER TABLE crminternet_payroll ADD COLUMN paid_at DATETIME NULL",
        ];
...
if ($method === 'POST' && ($action === 'upsert' || $action === '')) {
    ...
    $sql = "INSERT INTO crminternet_payroll
        (id, user_id, username, period, base_salary, hours_worked, hourly_rate, bonus, deductions, total, notes, created_at, updated_at)
        VALUES (:id, :uid, :un, :pe, :bs, :hw, :hr, :bo, :de, :to, :no, NOW(), NOW())
