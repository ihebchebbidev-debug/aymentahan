...
function ensure_external_agents(PDO $db): void {
    try {
        ...
        $repair = [
            "ALTER TABLE crminternet_external_agents CHANGE COLUMN name full_name VARCHAR(160) NOT NULL",
            "ALTER TABLE crminternet_external_agents ADD COLUMN full_name VARCHAR(160) NOT NULL DEFAULT ''",
            "ALTER TABLE crminternet_external_agents ADD COLUMN cin VARCHAR(40) NOT NULL DEFAULT ''",
            "ALTER TABLE crminternet_external_agents ADD COLUMN fixed_amount DECIMAL(10,2) NOT NULL DEFAULT 0",
        ];
...
if ($method === 'POST') {
    ...
    $s = $db->prepare("INSERT INTO crminternet_external_agents
        (id, full_name, phone, email, cin, commission_rate, fixed_amount, active, notes, created_at)
        VALUES (:id, :n, :p, :e, :c, :cr, :fa, :a, :no, NOW())");
    $s->execute([
        ':id' => $id, ':n' => $name,
        ':p'  => trim($in['phone'] ?? ''),
        ':e'  => trim($in['email'] ?? ''),
