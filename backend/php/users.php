...
function fetch_users_list(PDO $db): array
{
    ...
    $prospectCols = [];
    try {
        foreach ($db->query('SHOW COLUMNS FROM crminternet_prospects') as $col) {
            $prospectCols[$col['Field']] = true;
        }
    } catch (Throwable $e) {
        $prospectCols = [];
    }

    $canStats = isset($prospectCols['assigned_to']) && isset($prospectCols['outcome']);
    ...
    if ($canStats) {
        try {
            ...
            /* fall through to simple list */
        }
    }

    return $db->query(
        'SELECT u.*, 0 AS leads_handled, 0 AS contracts_won FROM crminternet_users u ORDER BY u.full_name'
    )->fetchAll();
}
