...
if ($method === 'POST') {
    ...
    if ($action === 'bulk') {
        ...
        if ($op === 'status') {
            if (!user_has_permission($db, $me, 'prospect.edit')) require_permission($db, $me, 'prospect.status');
            $st_v = trim($in['status'] ?? '');
        ...
        if ($op === 'type') {
            if (!user_has_permission($db, $me, 'prospect.edit')) require_permission($db, $me, 'prospect.type');
            $tid = $in['typeId'] ?? null;
            if ($tid === '') $tid = null;
            if ($tid !== null) {
...
if ($method === 'PATCH' || $method === 'PUT') {
    ...
    if (isset($needsSource) ? $needsSource : false) {
        if (!$canEdit) require_permission($db, $me, 'prospect.source');
    }
    if (isset($needsStatus) ? $needsStatus : false) {
        if (!$canEdit) require_permission($db, $me, 'prospect.status');
    }
    if (isset($needsAssign) ? $needsAssign : false) {
        if (!$canEdit) require_permission($db, $me, 'prospect.assign');
    }
    if (isset($needsEdit) ? $needsEdit : false) {
