...
const PROD_FALLBACK = "https://crm.ttshop.pro/code_source/backend/php";
...
export async function api<T = any>(path: string, opts: ApiOptions = {}): Promise<T> {
  ...
  if (data === null || data === undefined || typeof data !== "object") {
    throw new ApiError(
      `Réponse serveur vide ou invalide (${res.status}). Vérifiez que le backend PHP répond en JSON.`,
      res.status || 502,
    );
  }
  return data as T;
