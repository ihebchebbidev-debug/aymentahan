...
// Backend endpoints accept ?count=1 (just total) and ?page=N&per_page=K
...
// Tuned for high-volume datasets (500k+ rows).
...
/** Load every row from limit/offset APIs (reclamations, audit, guichet, …). */
export async function fetchAllLimitOffset<T>(
  endpoint: string,
  itemsKey: string,
  opts: {
    baseQuery?: Record<string, string | number>;
  ...
  const pageSize = opts.pageSize ?? DEFAULT_PER_PAGE;
  const all: T[] = [];
  let offset = 0;
  for (let guard = 0; guard < MAX_PAGES; guard++) {
    if (opts.signal?.aborted) throw new DOMException("aborted", "AbortError");
    const r = await api<Record<string, unknown>>(endpoint, {
      query: { ...opts.baseQuery, limit: pageSize, offset },
      signal: opts.signal,
    });
    const raw = r?.[itemsKey];
    const items = Array.isArray(raw) ? (raw as T[]) : [];
    ...
    const hasMore = Boolean(r?.has_more ?? (items.length >= pageSize));
