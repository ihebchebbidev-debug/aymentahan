...
export const PERMISSION_SECTIONS: PermissionSection[] = [
  {
    ...
    perms: [
      ...
      { key: "page.migrations", label: "Page : Migrations" },
  ...
  },
  {
    ...
    perms: [
      ...
      { key: "contract.type", label: "Modifier le type du contrat" },
  ...
  },
  {
    ...
    perms: [
      ...
      { key: "migration.assign", label: "Réassigner migration" },
      { key: "migration.type", label: "Modifier le type de migration" },
      { key: "migration.delete", label: "Supprimer migration" },
      { key: "migration.validate", label: "Valider migration" },
      { key: "migration.revert", label: "Renvoyer → Opportunité" },
      { key: "migration.export", label: "Exporter migrations" },
      { key: "migration.import", label: "Importer migrations" },
      { key: "migration.stages", label: "Gérer les étapes migration" },
    ],
  },
  {
...
/** Migration module keys — keep in sync with backend crm_migration_permission_keys(). */
export const MIGRATION_PERMISSION_KEYS = [
  "page.migrations",
  "migration.view",
  "migration.add",
  "migration.edit",
...
/** Route guard uses page.migrations; API accepts migration.view OR page.migrations. */
export function canAccessMigrationsPage(hasPermission: (key: string) => boolean): boolean {
  return hasPermission("page.migrations");
}

export function canViewMigrationsData(hasPermission: (key: string) => boolean): boolean {
