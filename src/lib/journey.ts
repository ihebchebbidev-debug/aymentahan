...
import { DEFAULT_PER_PAGE } from "./paginatedFetch";
...
export type JourneyEntity = "prospect" | "opportunity" | "contract" | "migration" | "system";
...
const ACTION_LABELS: Record<string, string> = {
  ...
  convert_migration: "Conversion → Migration",
...
async function fetchAudit(entity: JourneyEntity, id: string): Promise<JourneyEvent[]> {
  ...
  try {
    const r = await api<{ logs: AuditRow[] }>("/audit_log.php", {
      query: { entity, entity_id: id, limit: DEFAULT_PER_PAGE, sort: "asc" },
    });
    return (r.logs ?? []).map((l) => {
      let parsed: any = null;
...
async function fetchActivity(entity: JourneyEntity, id: string): Promise<JourneyEvent[]> {
  ...
  try {
    const r = await api<{ activity: ActivityRow[] }>(
      `/activity.php?entity=${encodeURIComponent(entity)}&entity_id=${encodeURIComponent(id)}&limit=${DEFAULT_PER_PAGE}`,
    );
...
export async function loadProspectJourney(args: {
  ...
  if (args.migrationId) {
    tasks.push(fetchAudit("migration", args.migrationId));
    tasks.push(fetchActivity("migration", args.migrationId));
  }
  ...
  return {
    ...
    migrationId: args.migrationId ?? null,
    events,
  };
}

/** Convenience: derive linked ids from store before loading. */
export type JourneyLinks = {
  opportunityId: string | null;
  contractId: string | null;
  migrationId?: string | null;
};
