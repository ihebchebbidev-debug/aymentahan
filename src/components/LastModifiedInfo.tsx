...
type Props =
  ...
  | { kind: "migration"; id: string; createdAt?: string | null; createdBy?: string | null };

// Field/action labels FR
const FIELD_FR: Record<string, string> = {
  status: "Statut", stage: "Étape", billing_status: "Statut facturation",
  billingStatus: "Statut facturation", workflowStatus: "Statut workflow",
  technicalStatus: "Statut technique", oldOperator: "Ancien opérateur",
  newOperator: "Nouvel opérateur", portingNumber: "N° portabilité",
  assigned_to: "Assigné à", assignedTo: "Assigné à",
...
export function LastModifiedInfo(props: Props) {
  ...
  useEffect(() => {
    ...
    const actQs = props.kind === "contract"
      ...
      : `?entity=${props.kind}&entity_id=${encodeURIComponent(props.id)}&limit=1`;
