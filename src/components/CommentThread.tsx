  ...
  return (
    <div className="space-y-3">
      {entries.length === 0 ? (
        ...
        entries.map((entry) => (
          <div key={entry.id} className="rounded-2xl border border-border bg-background shadow-sm p-4">
            ...
            <div className="whitespace-pre-wrap text-sm text-foreground leading-6 mt-3">{entry.body}</div>
          </div>
        ))
      )}
    </div>
