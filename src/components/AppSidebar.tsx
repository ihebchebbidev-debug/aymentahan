...
export const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { title: "Migrations", url: "/migrations", icon: ArrowRightLeft },
          ...
          {items.map((item) => {
            ...
            return (
              <li key={item.url}>
                ...
                {item.url === "/migrations" && <MigrationStagesSubmenu />}
...
function MigrationStagesSubmenu() {
  const stages = useMigrationStages();
  const currentPath = useRouterState({ select: (s) => s.location.pathname });
  const currentSearch = useRouterState({ select: (s) => s.location.search as { statut?: string } });
  const onMigrations = currentPath.startsWith("/migrations");
  const activeStage = onMigrations ? currentSearch?.statut : undefined;

  const storageKey = "sidebar.migrationStages.open";
  const [open, setOpen] = useState<boolean>(false);
  useEffect(() => {
    if (activeStage) { setOpen(true); return; }
    try {
