...
const ENTITY_LABEL: Record<string, string> = { prospect: "Lead", opportunity: "Opportunité", contract: "Contrat", migration: "Migration", system: "Système" };
...
export function JourneyTimeline({ prospectId, opportunityId, contractId, migrationId, bare = false }: JourneyTimelineProps) {
  ...
  const load = async () => {
    ...
    try {
      const b = await loadProspectJourney({ prospectId, opportunityId, contractId, migrationId });
      setBundle(b);
    } catch (e: any) { setErr(e?.message ?? "Erreur de chargement"); }
    finally { setLoading(false); }
  };

  useEffect(() => { void load(); /* eslint-disable-next-line */ }, [prospectId, opportunityId, contractId, migrationId]);
