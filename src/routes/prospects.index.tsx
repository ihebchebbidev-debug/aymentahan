...
const PROSPECT_IMPORT_FIELDS: ImportField[] = [
  ...
  { key: "source",     label: "Type source", sample: "Terrain" },
...
const statusColor: Record<string, string> = {
  ...
  "deja migré": "bg-destructive/10 text-destructive border-destructive/20",
  "facture impayé": "bg-destructive/10 text-destructive border-destructive/20",
...
function ProspectsPage() {
  ...
  const reset = async () => {
    if (!(await confirmDialog({ title: "Réinitialiser les filtres", description: "Effacer tous les filtres actifs (dates, champs personnalisés) et rétablir les filtres rapides ?", tone: "warning", confirmText: "Réinitialiser" }))) return;
  ...
  const baseColumns: DataGridColumn<Prospect>[] = [
    ...
    { key: "animateur", header: "Animateur", accessor: (p) => p.animateur ?? "", hideBelow: "lg",
      editor: ({ value, setValue }) => <CellInput value={value ?? ""} setValue={setValue} /> },
    {
      key: "source", header: "Type source", accessor: (p) => p.source ?? "", hideBelow: "lg",
          ...
          <div className="flex flex-col gap-2">
            ...
            <DynamicFilterBar
              ...
              onChange={(k, v) => {
                ...
                else if (k === "typeF") setTypeF(v || ALL);
                else if (k === "recoveredF") setRecoveredF(v || ALL);
          ...
          <div className="mt-3 pt-3 border-t border-border flex flex-wrap items-center gap-2">
            {customDefs.length > 0 && (
                <>
                  ...
                  <span className="text-[11px] text-muted-foreground italic">Gérés via Filtres dynamiques</span>
                </>
              )}
          </div>
        </Card>
