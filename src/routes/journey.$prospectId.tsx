...
function JourneyPage() {
  ...
  const convertToMigration = async () => {
    if (!opp) return;
    if (opp.convertedToMigration && opp.migrationId) {
      navigate({ to: "/migrations/$migrationId", params: { migrationId: opp.migrationId } });
      return;
    }
    if (!(await confirmDialog({ title: "Conversion migration", description: "Convertir en dossier migration (terminal) ?", tone: "info", confirmText: "Convertir" }))) return;
    try {
      setConverting(true);
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {opp?.convertedToMigration && opp.migrationId && (
              <Button size="sm" variant="outline" onClick={() => navigate({ to: "/migrations/$migrationId", params: { migrationId: opp.migrationId! } })}>
                <ArrowRightLeft className="h-4 w-4 mr-1.5" />Voir migration
              </Button>
            )}
        ...
        {!showFullJourney ? (
          ...
          <div className="space-y-3">
            ...
            <JourneyTimeline
              ...
              contractId={contract?.id ?? opp?.contractId ?? null}
