...
import { useAuth, type EmailChangeRequired, type OtpChallenge } from "@/lib/auth";
import { OtpStep } from "@/components/OtpStep";
import { LoginEmailSetupStep } from "@/components/LoginEmailSetupStep";
...
function LoginPage() {
  ...
  const [emailSetup, setEmailSetup] = useState<EmailChangeRequired | null>(null);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLockUntil(0);
      persistThrottle(0, 0);
      if (result.kind === "emailChange") {
        setSavedPassword(p);
        setEmailSetup(result.data);
        return;
      }
      if (result.kind === "otp") {
  ...
  const onEmailSetup = async (newEmail: string) => {
    setError(null);
    const u = (usernameRef.current?.value ?? username).trim();
    const p = savedPassword || passwordRef.current?.value || password;
    if (!u || !p) {
    ...
    setSubmitting(true);
    try {
      const result = await login(u, p, newEmail);
      if (result.kind === "otp") {
        setEmailSetup(null);
        setOtpChallenge(result.challenge);
        return;
      ...
      setError(err?.message ?? "Impossible d'enregistrer l'email.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="relative min-h-screen w-full flex items-center justify-center p-4 bg-background overflow-hidden">
      ...
      <div className="relative w-full max-w-[400px]">
        {otpChallenge ? (
          ...
          <LoginEmailSetupStep
            currentEmail={emailSetup.currentEmail}
            message={emailSetup.message}
            submitting={submitting}
            error={error}
            onBack={() => { setEmailSetup(null); setError(null); setSavedPassword(""); }}
            onSubmit={onEmailSetup}
          />
        ) : (
        <>
          <div className="space-y-1.5 text-center mb-7">
            <h1 className="text-[26px] font-semibold tracking-tight">Connexion</h1>
            <p className="text-sm text-muted-foreground">Connectez-vous à votre espace. Un code de vérification sera envoyé à votre adresse email.</p>
          </div>
