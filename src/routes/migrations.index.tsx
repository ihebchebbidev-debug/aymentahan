...
function MigrationsPage() {
  ...
  const BASE_COLS_META: { key: string; label: string }[] = [
    ...
    { key: "completedDate",  label: "Complétée" },
  ...
  const BASE_EXPORT_LABELS: Record<string, string> = {
    name: "Nom", firstName: "Prénom", civility: "Civilité",
    phone: "Téléphone", phone2: "Téléphone 2", cin: "CIN", birthDate: "Date de naissance",
    email: "E-mail", city: "Ville", gouvernorat: "Gouvernorat", delegation: "Délégation",
    zone: "Zone", address: "Adresse", codePostal: "Code postal", localisationXy: "Localisation (lat,lng)",
    animateur: "Animateur", ancienLigne: "Ancienne ligne", leadStatus: "Statut lead",
    oldOperator: "Ancien opérateur", newOperator: "Nouvel opérateur",
    portingNumber: "Portabilité", migrationType: "Type migration",
    workflowStatus: "Statut", technicalStatus: "Technique",
    externalRef: "Réf. externe", source: "Type source",
  ...
  const allColumns: DataGridColumn<Migration>[] = [
    { key: "civility", header: "Civ.", cell: (m) => <span className="text-muted-foreground">{m.civility || "—"}</span> },
    {
      key: "name",
      header: "Client",
      cell: (m) => (
    ...
    { key: "firstName", header: "Prénom", cell: (m) => <span className="text-muted-foreground">{m.firstName || "—"}</span> },
    { key: "phone", header: "Téléphone", cell: (m) => <span className="text-muted-foreground">{m.phone || "—"}</span> },
    { key: "phone2", header: "Tél. 2", cell: (m) => <span className="text-muted-foreground">{m.phone2 || "—"}</span> },
    { key: "cin", header: "CIN", cell: (m) => <span className="text-muted-foreground">{m.cin || "—"}</span> },
    { key: "birthDate", header: "Naissance", cell: (m) => <span className="text-muted-foreground">{m.birthDate || "—"}</span> },
    { key: "email", header: "E-mail", cell: (m) => <span className="text-muted-foreground">{m.email || "—"}</span> },
    { key: "gouvernorat", header: "Gouvernorat", cell: (m) => <span className="text-muted-foreground">{m.gouvernorat || "—"}</span> },
    { key: "delegation", header: "Délégation", cell: (m) => <span className="text-muted-foreground">{m.delegation || "—"}</span> },
    { key: "city", header: "Ville", cell: (m) => <span className="text-muted-foreground">{m.city || "—"}</span> },
    { key: "zone", header: "Zone", cell: (m) => <span className="text-muted-foreground">{m.zone || "—"}</span> },
    { key: "address", header: "Adresse", cell: (m) => <span className="text-muted-foreground truncate">{m.address || "—"}</span> },
    { key: "codePostal", header: "CP", cell: (m) => <span className="text-muted-foreground">{m.codePostal || "—"}</span> },
    { key: "localisationXy", header: "GPS", cell: (m) => <span className="text-muted-foreground truncate">{m.localisationXy || "—"}</span> },
    { key: "animateur", header: "Animateur", cell: (m) => <span className="text-muted-foreground">{m.animateur || "—"}</span> },
    { key: "ancienLigne", header: "Anc. ligne", cell: (m) => <span className="text-muted-foreground">{m.ancienLigne || "—"}</span> },
    { key: "leadStatus", header: "Statut lead", cell: (m) => <span className="text-muted-foreground">{m.leadStatus || "—"}</span> },
    ...
    { key: "migrationType", header: "Type migration", cell: (m) => <span className="text-muted-foreground">{m.migrationType || "—"}</span> },
    {
      key: "workflowStatus",
      header: "Statut",
      cell: (m) => (
    ...
    { key: "externalRef", header: "Réf. externe", cell: (m) => <span className="text-muted-foreground">{m.externalRef || "—"}</span> },
    { key: "source", header: "Type source", cell: (m) => <span className="text-muted-foreground">{m.source || "—"}</span> },
    { key: "assignedTo", header: "Assigné", cell: (m) => m.assignedTo || "—" },
    { key: "notes", header: "Notes", cell: (m) => <span className="text-muted-foreground truncate">{m.notes || "—"}</span> },
    { key: "comment1", header: "Obs. 1", cell: (m) => <span className="text-muted-foreground truncate">{m.comment1 || "—"}</span> },
    { key: "comment2", header: "Obs. 2", cell: (m) => <span className="text-muted-foreground truncate">{m.comment2 || "—"}</span> },
    { key: "requestedDate", header: "Demandée", cell: (m) => m.requestedDate?.slice(0, 10) ?? "—" },
    { key: "completedDate", header: "Complétée", cell: (m) => m.completedDate?.slice(0, 10) ?? "—" },
  ...
  const baseExportRows: Record<string, unknown>[] = filtered.map((m) => ({
    ...
    Prénom: m.firstName,
    Téléphone: m.phone ?? "",
    "Téléphone 2": m.phone2 ?? "",
    CIN: m.cin ?? "",
    "Date de naissance": m.birthDate ?? "",
    "E-mail": m.email ?? "",
    Gouvernorat: m.gouvernorat ?? "",
    Délégation: m.delegation ?? "",
    Ville: m.city ?? "",
    Zone: m.zone ?? "",
    Adresse: m.address ?? "",
    "Code postal": m.codePostal ?? "",
    "Localisation (lat,lng)": m.localisationXy ?? "",
    Animateur: m.animateur ?? "",
    "Ancienne ligne": m.ancienLigne ?? "",
    "Statut lead": m.leadStatus ?? "",
    "Ancien opérateur": m.oldOperator ?? "",
    "Nouvel opérateur": m.newOperator ?? "",
    Portabilité: m.portingNumber ?? "",
    "Type migration": m.migrationType ?? "",
    Statut: m.workflowStatus,
    Technique: m.technicalStatus ?? "",
    "Réf. externe": m.externalRef ?? "",
    Source: m.source ?? "",
    "Assigné à": m.assignedTo ?? "",
    Notes: m.notes ?? "",
    "Observation 1": m.comment1 ?? "",
    "Observation 2": m.comment2 ?? "",
    "Date demande": m.requestedDate ?? "",
    "Date complétée": m.completedDate ?? "",
