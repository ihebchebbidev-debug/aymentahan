...
function OpportunityDetailPage() {
  ...
  const isTerminal = !!(opp.convertedToContract || opp.convertedToMigration);
  ...
  const convertMigration = async () => {
    if (opp.convertedToMigration && opp.migrationId) {
      navigate({ to: "/migrations/$migrationId", params: { migrationId: opp.migrationId } });
      return;
    }
    if (!(await confirmDialog({ title: "Conversion migration", description: "Convertir en dossier migration ? (étape terminale, pas de contrat ensuite en v1)", tone: "info", confirmText: "Convertir" }))) return;
    try {
      setBusy(true);
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            {canConvertMigration && !isTerminal && (
              <Button size="sm" variant="outline" className="border-primary/40 text-primary hover:bg-primary/10" disabled={busy} onClick={convertMigration}>
            ...
            {opp.convertedToMigration && opp.migrationId && (
              <Button size="sm" variant="outline" onClick={() => navigate({ to: "/migrations/$migrationId", params: { migrationId: opp.migrationId! } })}>
                <ArrowRightLeft className="h-4 w-4 mr-1.5" />Voir la migration
              </Button>
            )}
