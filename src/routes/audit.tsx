...
import { fetchAllLimitOffset } from "@/lib/paginatedFetch";
...
function AuditPage() {
  ...
  const exportAllCsv = async () => {
    if (!API_ENABLED) return;
    try {
      const logs = await fetchAllLimitOffset<LogRow>("/audit_log.php", "logs", {
        baseQuery: {
          from: from || undefined, to: to || undefined,
          user: fUser !== "all" ? fUser : undefined,
          role: fRole !== "all" ? fRole : undefined,
      ...
      exportCSV(`audit_full_${new Date().toISOString().slice(0, 10)}.csv`, rowsToCsv(logs));
