...
import {
  ...
  ClipboardList, CheckSquare, UserPlus, Layers, Building2, Hourglass, ArrowRightLeft,
} from "lucide-react";
import {
...
import { AGENT_ROLES } from "@/lib/roleLabels";
...
function Dashboard() {
  ...
  const canViewMigrations = canViewMigrationsData(hasPermission);
  const myUsername = user?.username ?? "";
  // Visibility policy: any user who can reach the dashboard sees ALL data
  // for the modules they have permission to view. No team / assignedTo
  // scoping — consistent with the list pages and the backend (which never
  // scopes GET queries by team or agent). Access is gated purely by the
  // page.* permissions enforced upstream by RequirePerm.
  ...
  type MigrationRow = { createdAt?: string | null; requestedDate?: string | null; assignedTo?: string };
  const [migrations, setMigrations] = useState<MigrationRow[]>([]);
  useEffect(() => {
    if (!API_ENABLED || !canViewMigrations) return;
    fetchAllPaginated<MigrationRow>(
      "/migrations.php",
      "migrations",
      { baseQuery: { _t: Date.now() } },
    )
  ...
  const migrationDateKey = (m: MigrationRow) => (m.requestedDate ?? m.createdAt ?? "").slice(0, 10);
  const migrationsToday = visibleMigrations.filter((m) => migrationDateKey(m) === todayStr).length;
  const migrationsMonth = visibleMigrations.filter((m) => migrationDateKey(m).slice(0, 7) === monthStr).length;

  const heroKpiCols = canViewMigrations
  ...
  // Équipes dynamiques (Backoffice → Équipes). Fallback sur les valeurs
  // déjà présentes sur les utilisateurs si l'API teams n'est pas dispo.
  const { teams } = useTeams();
  const agencyOptions = useMemo(() => {
    const set = new Set<string>();
    teams.forEach((t) => { if (t?.name) set.add(t.name); });
    if (set.size === 0) {
  ...
  // Visibility policy: any user who can reach the dashboard sees data from
  // ALL teams by default. The "agency" dropdown remains as a user-driven
  // filter, but is never auto-locked to the user's own team. Access control
  // is handled upstream by the page.* permissions (RequirePerm).
